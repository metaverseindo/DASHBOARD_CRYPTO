...
st.markdown("""
    ...
    /* Memaksa angka rata kanan & font monospace agar sejajar */
...
def fetch_final_data():
    try:
        ...
        for sym, v in tickers.items():
            try:
                if '/USDT' in sym and v.get('last') and v.get('quoteVolume'):
                    ...
                    # Sparkline enteng (5 titik data) - Murni angka
                    p = float(v['last'])
                    c = float(v.get('percentage', 0) or 0)
                    trend = [p * (1 + (c / 100) * (i / 4)) for i in range(5)]
                    
                    # Logo via Google Proxy (Paling stabil buat server)
        ...
        # Sort & Ambil Top 40 (Jumlah ideal biar gak crash)
...
st.markdown("""
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 15px 25px; background: #0f172a; border-radius: 12px; border: 1px solid #1e293b; margin-bottom: 20px;">
        <div>
            ...
            <p style="color: #64748b; margin: 0; font-size: 11px; font-family: monospace;">BLOCKCHAIN TERMINAL v3.5</p>
        </div>
        <div style="color: #10b981; font-family: monospace; font-size: 12px;">● LIVE CONNECTION</div>
    </div>
    """, unsafe_allow_html=True)
...
if not df.empty:
    ...
    st.info("🔄 Sedang narik data... Kalau tetep blank, coba refresh browser lu sekali lagi.")
