...
st.markdown("""
    ...
    /* Memaksa teks di tabel rata tengah & angka rata kanan */
...
# 3. DATA ENGINE (VERSION 18 - ULTRA STABLE)
...
def fetch_pro_data():
    try:
        ...
        for sym, v in tickers.items():
            try:
                # Filter hanya USDT & pastikan data krusial ada
                if '/USDT' in sym and v.get('last') and v.get('quoteVolume'):
                    ...
                    # Pastikan data trend murni float list
                    trend = [float(p * (1 + (c / 100) * (i / 5))) for i in range(6)]
                    
                    # Logo via Google Proxy (Paling aman dari blokir server)
        ...
        # Ambil Top 40 biar gak berat pas render
...
st.markdown("""
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 15px 25px; background: #0f172a; border-radius: 12px; border: 1px solid #1e293b; margin-bottom: 20px;">
        <div>
            ...
            <p style="color: #64748b; margin: 0; font-size: 11px; font-family: monospace;">REAL-TIME TERMINAL v4.0</p>
        </div>
...
if not df.empty:
    # 5. RENDER TABEL (KUNCI FORMAT & WIDTH)
    st.dataframe(
        df,
        column_config={
    ...
    st.caption(f"Sync: {datetime.now(tz).strftime('%H:%M:%S')} WIB | Source: KuCoin Global | Auto-refresh: 15s")
else:
    st.warning("🔄 Sedang narik data market... Jika blank, silakan refresh browser lu.")
