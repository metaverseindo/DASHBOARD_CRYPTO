...
st.markdown("""
    ...
    /* Memperbaiki tinggi baris agar logo dan chart sejajar */
...
def fetch_market_data():
    try:
        # Gunakan KuCoin sebagai sumber data utama
        ...
        for sym, v in tickers.items():
            if '/USDT' in sym and v['last'] is not None and v['quoteVolume'] > 0:
                ...
                # Setup Sparkline (Trend) yang aman dari Error
                current_price = float(v['last'])
                change_pct = float(v['percentage'] or 0.0)
                # Buat 10 titik data sederhana untuk grafik
        ...
        # Jika API Error, return dataframe kosong agar tidak crash
        return pd.DataFrame()
...
st.markdown("""
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 20px; background: #0f172a; border-radius: 15px; border: 1px solid #1e293b; margin-bottom: 20px;">
        ...
        <div style="text-align: right;">
            <span style="color: #10b981; font-weight: bold; font-family: monospace; font-size: 12px;">● LIVE CONNECTION</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if not df.empty:
    # 5. RENDER TABEL DENGAN COLUMN_CONFIG (LEBIH STABIL DARI .STYLE)
    st.dataframe(
        df,
        column_config={
    ...
    st.caption(f"Last Sync: {datetime.now(tz).strftime('%H:%M:%S')} WIB | Auto-refresh every 15s")
else:
    st.info("🔄 Connecting to Kucoin Global... If this takes too long, please refresh the page.")
